...
from backend.schemas import ChatConversationOut, ChatConversationCreate, ChatParticipantOut, ChatMessageOut, ChatMessageCreate
    ...
    for part in my_participations:
        ...
        if last_msg:
            last_message = last_msg.text if last_msg.msg_type == "text" else {
                "image": "📷 Bild", "file": "📎 Datei", "voice": "🎤 Sprachnachricht",
...
async def list_messages(conversation_id: int, before: Optional[int] = None, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return [
        ChatMessageOut(
            ...
            file_name=m.file_name, file_size=m.file_size, mime_type=m.mime_type, created_at=m.created_at,
        ) for m in messages
    ]
...
async def send_message(conversation_id: int, data: ChatMessageCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if data.msg_type != "text" and not data.storage_filename:
    ...
    others_result = await db.execute(select(User).join(
    ...
    if others:
        ...
        try:
            await push_to_users(db, others, title=current_user.name, body=preview or "Neue Nachricht")
    ...
    return ChatMessageOut(
        ...
        file_name=msg.file_name, file_size=msg.file_size, mime_type=msg.mime_type, created_at=msg.created_at,
    )
